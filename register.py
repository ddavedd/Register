...
class Register:
    ...
    def update_info_from_database(self):
        ...
        ITEM, PROD, CAT, PROD_CAT, PROD_PRICE, DEAL = list(range(6))
    ...
    def update_products_frame(self):
        ...
        for product in enabled_products:
           ...
           product_button.grid(row=int(prod_row), column=prod_col)
           product_index += 1
    ...
    def create_product_button(self, product):
       ...
       # How the product is displayed in the product frame
        ...
        try:
            ...
            tkinter.messagebox.showerror("Color not allowed", "The color %s for product %s is not allowed, check spelling" % (product.color, product.name))
    ...
    def update_category_frame(self):
        ...
        for category in [c for c in self.categories if c.enabled]:
            ...
            try:
                ...
                tkinter.messagebox.showerror("Color not allowed", "The color %s for category %s is not allowed, check spelling" % (category.color, category.name))
            ...
            cat_button.grid(row=int(cat_row), column=cat_col)
            category_index += 1
    ...
    def get_weight(self):
        ...
        if self.scale is None:
            weight = tkinter.simpledialog.askfloat("Weight Entry", "Weight (lbs):")
        else:
            try:
    ...
    def add_to_cart(self, product):
        ...
        if product.is_by_weight:
            ...
            # get price
            premarked_price = tkinter.simpledialog.askfloat("Premarked Price", "Price:")
            ...
            if self._shift_is_pressed:
               amount = tkinter.simpledialog.askinteger("Adding how many?", "Amount:")
               if amount is None:
    ...
    def update_cart_items(self):
        ...
        for text_headers, text_index in zip(text_headers, list(range(len(text_headers)))):
    ...
    def update_cart_totals(self):
        ...
        totals_font = tkinter.font.Font(family="Tahoma", size=20)
        totals_font_bold = tkinter.font.Font(family="Tahoma", size=20, weight='bold')
    ...
    def update_payment_frame(self):
        ...
        payment_font = tkinter.font.Font(weight=tkinter.font.BOLD, size=self.payment_font_size)
    ...
    def no_sale(self):
        if self._shift_is_pressed:
            tkinter.messagebox.showwarning("Diagnostic Mode", "Entering Diagnostic Mode")
            self.diagnostic_mode = True
        elif self._ctrl_is_pressed:
            tkinter.messagebox.showwarning("Receipt Read Mode", "Entering Receipt Read Mode")
    ...
    def cash_pay(self):
        ...
        if total > 0.00:
            ...
            tkinter.messagebox.showwarning("Total is zero", "Total is zero, did you mean No Sale?")
    ...
    def input_cash_amount_dialog(self):
        ...
        amount_index = 0
        bold_font = tkinter.font.Font(family="Arial", size=12, weight=tkinter.font.BOLD)
        ...
        entry_font = tkinter.font.Font(size=20, weight=tkinter.font.BOLD)
    ...
    def submit_cash_amount(self, is_string_var, amount, window_to_close):
        ...
        try:
            ...
            print(amount)
            tkinter.messagebox.showwarning("Cash not right", "Can't convert amount to a number")
        else:
            total, _, _, _ = self.get_total_price()
    ...
    def display_change_amount(self, total, cash_amount):
        ...
        tkinter.messagebox.showinfo("Change", "Change: $%.2f" % change)
    ...
    def credit_pay(self):
        ...
        if total > 0.00:
            ...
            self.simple_lock()
            tkinter.messagebox.showinfo("Credit Card Payment", "Total: $%.2f" % (total))
            self.simple_unlock()
            self.clear_cart()
    ...
    def check_pay(self):
        ...
        if total > 0.00:
            ...
            self.simple_lock()
            tkinter.messagebox.showinfo("Check Payment", "Total: $%.2f" % (total))
            self.simple_unlock()
            self.clear_cart()
    ...
    def change_cart_amount(self, cart_row):
        ...
        if self.cart[cart_row].product.is_by_weight:
            amount = tkinter.simpledialog.askfloat("Enter new weight", "Weight:")
        elif self.cart[cart_row].product.is_premarked:
            amount = tkinter.simpledialog.askfloat("Enter new amount", "Amount:")
        else:
            amount = tkinter.simpledialog.askinteger("Enter new amount", "Amount:")
        self.simple_unlock()
    ...
    def read_products_categories_constants(self):
        ...
        # Set up fonts
        self.product_font = tkinter.font.Font(size=self.product_font_size, weight=tkinter.font.BOLD)
        self.category_font = tkinter.font.Font(size=self.category_font_size, weight=tkinter.font.BOLD)
        self.payment_font = tkinter.font.Font(size=self.payment_font_size, weight=tkinter.font.BOLD)
        self.cart_item_font = tkinter.font.Font(size=self.cart_item_font_size, weight=tkinter.font.BOLD)
        self.total_font = tkinter.font.Font(size=self.cart_total_font_size, weight=tkinter.font.BOLD)
    ...
    def __init__(self, master, init_file_name, scale):
        ...
        if cursor is None:
            tkinter.messagebox.showwarning("Database Error", "Could not connect to database, check errors on terminal")
            new_ip_address = tkinter.simpledialog.askstring("Manual ip address entry", "Enter ip address of host to connect to (127.0.0.1 if can't connect to network)")
            ...
            if new_ip_address is not None:
               ...
               if cursor is None:
                  tkinter.messagebox.showwarning("Manual ip entry failure", "Failed to connect to manual ip address, closing")
                  master.destroy()
                  return
            else:
               master.destroy()
...
def startRegister():
    ...
    if scale is None:
        if not possible_connection:
            tkinter.messagebox.showwarning("Scale Not Connected", "Scale is not connected.\
